...
# potato bookの数をscoreに
 ...
 execute if data entity @s equipment.feet.components."minecraft:custom_data".Armor store result storage temp: temp_setting.Potato int 1 run scoreboard players operation #PotatoBook.Amount Temporary *= #4 Constant

## REFORGE
# reforge Strがあるならそれを代入 ないなら""
execute if data entity @s equipment.feet.components."minecraft:custom_data".ReforgeStatus.Hp store result storage temp: temp_setting.Reforge int 1 run data get entity @s equipment.feet.components."minecraft:custom_data".ReforgeStatus.Hp

## The Art of War
function system:item/lore/feet/lore/calc_status/calc/health/detail/aop

## Gemstone
 execute store result score #Gem_1 Temporary run data get entity @s equipment.feet.components."minecraft:custom_data".GemstoneS_1.Hp
 execute store result score #Gem_2 Temporary run data get entity @s equipment.feet.components."minecraft:custom_data".GemstoneS_2.Hp
...
## 最終計算
 execute store result score #Base Temporary run data get entity @s equipment.feet.components."minecraft:custom_data".Stats.Health
 execute store result score #Reforge Temporary run data get entity @s equipment.feet.components."minecraft:custom_data".ReforgeStatus.Hp
...
function system:item/lore/feet/lore/calc_status/calc/health/storage with storage temp: temp_setting
# potatoがついていない場合
...
# reforgeがない場合
 execute unless data entity @s equipment.feet.components."minecraft:custom_data".ReforgeStatus.Hp run \
...
# gemがない場合
 execute unless data entity @s equipment.feet.components."minecraft:custom_data".GemstoneS_1.Hp unless data entity @s equipment.feet.components."minecraft:custom_data".GemstoneS_2.Hp run data modify storage temp: temp_lore.Gemstone set value ""


# lore生成
 execute if entity @s[tag=InDungeon] if score #Base Temporary matches 1.. run function system:item/lore/feet/lore/calc_status/store/health/dungeon with storage temp: temp_lore
 execute unless entity @s[tag=InDungeon] if score #Base Temporary matches 1.. run function system:item/lore/feet/lore/calc_status/store/health/main with storage temp: temp_lore

scoreboard players operation @s[tag=!InDungeon] feet.Hp = #Base Temporary
scoreboard players operation @s[tag=InDungeon] feet.Hp = #ForDungeon Temporary
