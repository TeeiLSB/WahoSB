...
execute if data entity @s equipment.feet.components."minecraft:custom_data".ReforgeStatus.Str store result storage temp: temp_setting.Reforge int 1 run data get entity @s equipment.feet.components."minecraft:custom_data".ReforgeStatus.Str

## The Art of War
function system:item/lore/feet/lore/calc_status/calc/strength/detail/aow

## Gemstone
 execute store result score #Gem_1 Temporary run data get entity @s equipment.feet.components."minecraft:custom_data".GemstoneS_1.Str
 execute store result score #Gem_2 Temporary run data get entity @s equipment.feet.components."minecraft:custom_data".GemstoneS_2.Str
...
## 最終計算
 execute store result score #Base Temporary run data get entity @s equipment.feet.components."minecraft:custom_data".Stats.Strength
 execute store result score #Reforge Temporary run data get entity @s equipment.feet.components."minecraft:custom_data".ReforgeStatus.Str
...
function system:item/lore/feet/lore/calc_status/calc/strength/storage with storage temp: temp_setting
# potatoがついていない場合
...
# reforgeがない場合
 execute unless data entity @s equipment.feet.components."minecraft:custom_data".ReforgeStatus.Str run \
...
# gemがない場合
 execute unless data entity @s equipment.feet.components."minecraft:custom_data".GemstoneS_1.Str unless data entity @s equipment.feet.components."minecraft:custom_data".GemstoneS_2.Str run data modify storage temp: temp_lore.Gemstone set value ""


# lore生成
 execute if entity @s[tag=InDungeon] if score #Base Temporary matches 1.. run function system:item/lore/feet/lore/calc_status/store/strength/dungeon with storage temp: temp_lore
 execute unless entity @s[tag=InDungeon] if score #Base Temporary matches 1.. run function system:item/lore/feet/lore/calc_status/store/strength/main with storage temp: temp_lore

scoreboard players operation @s[tag=!InDungeon] feet.Str = #Base Temporary
scoreboard players operation @s[tag=InDungeon] feet.Str = #ForDungeon Temporary
