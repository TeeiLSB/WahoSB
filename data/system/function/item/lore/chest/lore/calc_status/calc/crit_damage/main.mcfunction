...
execute if data entity @s equipment.chest.components."minecraft:custom_data".ReforgeStatus.Cd store result storage temp: temp_setting.Reforge int 1 run data get entity @s equipment.chest.components."minecraft:custom_data".ReforgeStatus.Cd

## Gemstone
 execute store result score #Gem_1 Temporary run data get entity @s equipment.chest.components."minecraft:custom_data".GemstoneS_1.Cd
 execute store result score #Gem_2 Temporary run data get entity @s equipment.chest.components."minecraft:custom_data".GemstoneS_2.Cd
...
## 最終計算
 execute store result score #Base Temporary run data get entity @s equipment.chest.components."minecraft:custom_data".Stats.CritDamage
 execute store result score #Reforge Temporary run data get entity @s equipment.chest.components."minecraft:custom_data".ReforgeStatus.Cd
...
# reforgeがない場合
 execute unless data entity @s equipment.chest.components."minecraft:custom_data".ReforgeStatus.Cd run \
...
# gemがない場合
 execute unless data entity @s equipment.chest.components."minecraft:custom_data".GemstoneS_1.Cd unless data entity @s equipment.chest.components."minecraft:custom_data".GemstoneS_2.Cd run data modify storage temp: temp_lore.Gemstone set value ""
...
scoreboard players operation @s[tag=!InDungeon] Chest.Cd = #Base Temporary
scoreboard players operation @s[tag=InDungeon] Chest.Cd = #ForDungeon Temporary
