...
execute if data entity @s equipment.head.components."minecraft:custom_data".ReforgeStatus.Fero store result storage temp: temp_setting.Reforge int 1 run data get entity @s equipment.head.components."minecraft:custom_data".ReforgeStatus.Fero

## 最終計算
 execute store result score #Base Temporary run data get entity @s equipment.head.components."minecraft:custom_data".Stats.Ferocity
 execute store result score #Reforge Temporary run data get entity @s equipment.head.components."minecraft:custom_data".ReforgeStatus.Fero
...
# reforgeがない場合
 execute unless data entity @s equipment.head.components."minecraft:custom_data".ReforgeStatus.Fero run \
...
scoreboard players operation @s[tag=!InDungeon] head.Fero = #Base Temporary
scoreboard players operation @s[tag=InDungeon] head.Fero = #ForDungeon Temporary
