...
execute if data entity @s equipment.head.components."minecraft:custom_data".ReforgeStatus.Cc store result storage temp: temp_setting.Reforge int 1 run data get entity @s equipment.head.components."minecraft:custom_data".ReforgeStatus.Cc

## 最終計算
 execute store result score #Base Temporary run data get entity @s equipment.head.components."minecraft:custom_data".Stats.CritChance
 execute store result score #Reforge Temporary run data get entity @s equipment.head.components."minecraft:custom_data".ReforgeStatus.Cc
...
# reforgeがない場合
 execute unless data entity @s equipment.head.components."minecraft:custom_data".ReforgeStatus.Cc run \
...
scoreboard players operation @s[tag=!InDungeon] head.Cc = #Base Temporary
scoreboard players operation @s[tag=InDungeon] head.Cc = #ForDungeon Temporary
