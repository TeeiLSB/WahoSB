...
# potato bookの数をscoreに
 ...
 execute if data block 0 0 0 Items[0].components."minecraft:custom_data".Armor store result storage temp: temp_setting.Potato int 1 run scoreboard players operation #PotatoBook.Amount Temporary *= #2 Constant

## REFORGE
# reforge Strがあるならそれを代入 ないなら""
execute if data block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Def store result storage temp: temp_setting.Reforge int 1 run data get block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Def



## Gemstone
 execute store result score #Gem_1 Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".GemstoneS_1.Def
 execute store result score #Gem_2 Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".GemstoneS_2.Def
...
## 最終計算
 execute store result score #Base Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".Stats.Defense
 execute store result score #Reforge Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Def
 execute store result score #StarAmount Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".Star
 function system:item/enchantment/ench_stats/calc/def/main
 execute store result score #Ench Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".EnchDef
# 足す
 scoreboard players operation #Base Temporary += #Reforge Temporary
 scoreboard players operation #Base Temporary += #Ench Temporary
 function system:item/enchantment/ench_stats/functional_enchantment/chimera/copy_stat {"s":"Def"}
...
# reforgeがない場合
 execute unless data block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Def run \
...
# gemがない場合
 execute unless data block 0 0 0 Items[0].components."minecraft:custom_data".GemstoneS_1.Def unless data block 0 0 0 Items[0].components."minecraft:custom_data".GemstoneS_2.Def run data modify storage temp: temp_lore.Gemstone set value ""
...
function system:item/lore/main/lore/calc_status/calc/apply_score {"var":"Def"}
