...
execute if data block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Cd store result storage temp: temp_setting.Reforge int 1 run data get block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Cd

## Gemstone
 execute store result score #Gem_1 Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".GemstoneS_1.Cd
 execute store result score #Gem_2 Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".GemstoneS_2.Cd
...
## 最終計算
 execute store result score #Base Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".Stats.CritDamage
 execute store result score #Reforge Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Cd
 execute store result score #StarAmount Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".Star
   function system:item/enchantment/ench_stats/calc/cd/main
 execute store result score #Ench Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".EnchCd
# 足す
 ...
 function system:item/enchantment/ench_stats/functional_enchantment/chimera/copy_stat {"s":"Cd"}
...
# reforgeがない場合
 execute unless data block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Cd run \
...
# gemがない場合
 execute unless data block 0 0 0 Items[0].components."minecraft:custom_data".GemstoneS_1.Cd unless data block 0 0 0 Items[0].components."minecraft:custom_data".GemstoneS_2.Cd run data modify storage temp: temp_lore.Gemstone set value ""
...
function system:item/lore/main/lore/calc_status/calc/apply_score {"var":"Cd"}
