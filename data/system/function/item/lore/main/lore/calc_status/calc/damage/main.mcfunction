...
execute if data block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Dmg store result storage temp: temp_setting.Reforge int 1 run data get block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Dmg

## 最終計算
 execute store result score #Base Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".Stats.BaseDamage
 execute store result score #Reforge Temporary run data get block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Dmg
...
# 足す
 ...
 function system:item/enchantment/ench_stats/functional_enchantment/chimera/copy_stat {"s":"Dmg"}
...
# reforgeがない場合
 execute unless data block 0 0 0 Items[0].components."minecraft:custom_data".ReforgeStatus.Dmg run \
...
function system:item/lore/main/lore/calc_status/calc/apply_score {"var":"Dmg"}
