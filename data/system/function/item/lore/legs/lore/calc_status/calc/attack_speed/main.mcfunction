...
execute if data entity @s equipment.legs.components."minecraft:custom_data".ReforgeStatus.As store result storage temp: temp_setting.Reforge int 1 run data get entity @s equipment.legs.components."minecraft:custom_data".ReforgeStatus.As

## 最終計算
 execute store result score #Base Temporary run data get entity @s equipment.legs.components."minecraft:custom_data".Stats.AttackSpeed
 execute store result score #Reforge Temporary run data get entity @s equipment.legs.components."minecraft:custom_data".ReforgeStatus.As
...
# reforgeがない場合
 execute unless data entity @s equipment.legs.components."minecraft:custom_data".ReforgeStatus.As run \
...
scoreboard players operation @s[tag=!InDungeon] legs.As = #Base Temporary
scoreboard players operation @s[tag=InDungeon] legs.As = #ForDungeon Temporary
