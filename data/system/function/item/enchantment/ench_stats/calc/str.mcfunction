# reset
 item modify block 0 0 0 container.0 {function: "set_custom_data",tag:{EnchStr:""}}

# set status

scoreboard players set #ench Temporary 0
 execute if items block 0 0 0 container.0 *[custom_data~{enchant:{the_one:7}}] run scoreboard players add #ench Temporary 15

execute store result storage temp: ench.value int 1 run scoreboard players get #ench Temporary
data modify storage temp: ench.s set value Str
